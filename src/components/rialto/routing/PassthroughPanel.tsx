 ...
 *
 * No live reading. State went because the scheduler reads only what a
 * route resolves to, and a passthrough surface names nothing, so the pill
 * read `unknown` on every row of every install. Tier went too: the caller
 * names the model itself here, so which tier a route would have called it
 * has no bearing on where the request goes. "Copy as list" went because
 * the per-row copy is the one that gets used.
 */
  ...
  return (
    <>
      {/* Band 3 of the passthrough half, at the same height and rule as
          the routed half's action band. It owns its gap: this heading is

 ...
 * Pro, whose own two levels work the way Claude's Max does. Percentages
 * from two seats are therefore not comparable quantities: a 20x sitting at
 * 0% carries twenty times the headroom a Pro at 0% does. Anything folding
 * several accounts into one number has to weight by this, or it reports a
 * pool of one spent 5x and one untouched 20x as half gone when four fifths
 ...
 * only "max" in the plan; Codex sends `chatgpt_plan_type` and leaves the
 * tier empty. Matching the "20x" / "5x" fragment wherever it appears means
 * a Codex account that starts naming its level lands on the right weight
 * without another release.
 *
 * `kind` is needed because "pro" means opposite ends of the range: Claude
 * Pro is the unit, Codex Pro is a Max-class seat. Without the vendor a
 * bare "pro" would have to guess, and both guesses are wrong half the time.
 *
 ...
 * Pure arithmetic on three strings — no imports, so the browser can carry it.
 */
...
export const planCapacityWeight = (kind: SeatKind, plan: string | null, rateLimitTier: string | null): number => {
  ...
  if (kind === 'codex' && said.includes('pro')) return 5
  return 1
}

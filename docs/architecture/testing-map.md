...
| `router.test.ts` | ティアマップ周りの部品。`requestedTierOf`（モデル名のファミリを大小文字無視で読み、`fable` が `opus` に勝ち、Claude 以外や名前無しは `other`）、`subscriptionKindOf`（429 をどのベンダの窓に付けるか）、`routeRequest` を通したサブエージェントタグ — 記録・除去されるがレーンは選ばず値も読まない、ルートが無いときも passthrough 面でも除去・記録される、閉じていないタグは存在扱いで残す、第 2 system ブロック以外・単一ブロックの system では印にならない |

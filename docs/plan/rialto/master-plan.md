...
| Tier routing（チェーン → ティアマップ） | **In Progress** | 計画は `docs/plan/quota-and-tier-routing.md`。ルーティングを「シナリオ × レーンごとの具体モデルのチェーン」から「要求ティア → (プロバイダ, ティアエイリアス)」の表（`TierRoute` / `ProviderTierAlias`）に作り直し、新モデルが出ても動かすのはエイリアス 1 行だけにした。シナリオ分類・agent / subagent レーン・`quota-router`・proactive failover・スケジューラの重みは削除し、スケジューラは対象ごとの quota snapshot を出すだけになった。旧チェーンは `db seed` の backfill が profile ごとに一度だけ変換する。P0-1〜P2-3（#523〜#531、モック #526）は PR 済み、P1 UI（#532）と P2-4+P2-5（切り替え）は draft、P2-6 はドキュメント（`CLAUDE.md` / README 3言語 / `docs/architecture/routing.md` ほか）。残: P2-7（旧テーブルを落とす縮退マイグレーション）は release A が本番で起動し全 profile に印が付いてから |
